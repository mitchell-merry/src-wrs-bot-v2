...
const LeaderboardRemoveCommand: Subcommand = {
    data: new SlashCommandSubcommandBuilder()
        ...
        .addIntegerOption(o =>
            o
                .setName('leaderboard')
                .setDescription('The leaderboard to remove.')
                .setRequired(true)
        ...
        .addBooleanOption(o =>
            o
                .setName('delete_role')
                .setDescription('Delete the associated role.'),
        ),
    perm: 'mods',
    execute: async interaction => {
        ...
        const [confirm] = await new ConfirmationMenu(message).spawnMenu(
            interaction,
            'EDIT_REPLY',
        );
        ...
        if (delete_role) {
            const boardsWithRole = await tlbRepo.find({
                where: { role_id: tlb.role_id },
            });
            if (boardsWithRole.length > 1)
                throw new UserError(
                    `Error: More than one leaderboard use the role for this board.`,
                );
        ...
        if (lb.trackedLeaderboards.length === 1)
            await lbRepo.delete(lbCrit.where);

        await interaction.editReply(`Removed the leaderboard ${lb.lb_name}.`);
    },
