...
const LeaderboardAddCommand: Subcommand = {
    data: new SlashCommandSubcommandBuilder()
        ...
        .addStringOption(o =>
            o
                .setName('game')
                .setDescription(
                    'The game abbreviation of the leaderboard to add.',
                )
                .setRequired(true),
        )
        .addRoleOption(o =>
            o
                .setName('role')
                .setDescription(
                    'The role to attach the leaderboard to. Leave blank to generate one.',
                ),
        ),
    perm: 'mods',
    execute: async (interaction, guildEnt) => {
        ...
        if (!gameOpt.match(gameRegex))
            throw new UserError(
                `Invalid game abbreviation: ${gameOpt}. You should use the 'day_seven' part of the game link 'https://www.speedrun.com/day_seven', for example.`,
            );
        ...
        let errorRoleName = '';
        if (!roleOpt) {
            // if no role was provided, we want to make a new role
            const above_role = await interaction.guild!.roles.fetch(
        ...
        if (interaction.guild!.members.me!.roles.highest.position < position) {
            throw new UserError(
                `Bot does not have permission to create/manage a role at this position. Give the bot a role higher than \`@${errorRoleName}\` [${
                    interaction.guild!.members.me!.roles.highest.position
                }, ${position}].`,
            );
        }

        const { game, category, variables, level } =
            await new LeaderboardMenu().spawnMenu(interaction, gameOpt);

        // build leaderboard name
        const labels = variables.map(
            ([subcat, v]) => subcat.values.values[v].label,
        );
        const lb_name = SRC.buildLeaderboardName(
        ...
        let board = await LeaderboardEntity.exists(
            game.id,
            category.id,
            variables,
            level?.id,
        ...
        const exists =
            board &&
            board.trackedLeaderboards.find(
                tlb => tlb.guild_id === interaction.guildId,
            );
        if (exists)
            throw new UserError(
        ...
        if (!board) {
            board = new LeaderboardEntity(
                game.id,
                category.id,
                lb_name,
                level?.id,
            );
            board.variables = variables.map(
                ([subcat, v]) => new VariableEntity(board!, subcat.id, v),
            );
            board.trackedLeaderboards = [];
        }
        board.trackedLeaderboards.push(
            new TrackedLeaderboardEntity(
                interaction.guildId!,
                board.lb_id,
                role!.id,
            ),
