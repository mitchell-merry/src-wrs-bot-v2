...
const LeaderboardSetroleCommand: Subcommand = {
    data: new SlashCommandSubcommandBuilder()
        ...
        .addIntegerOption(o =>
            o
                .setName('leaderboard')
                .setDescription('The leaderboard to change the role of.')
                .setRequired(true)
        ...
        .addRoleOption(o =>
            o
                .setName('role')
                .setDescription('The role to change to.')
                .setRequired(true),
        ),
    perm: 'mods',
    execute: async interaction => {
        ...
        if (
            ...
            throw new UserError(
                'Bot does not have permission to modify the specified role.',
            );

        tlb.role_id = new_role.id;
        await tlbRepo.save(tlb);
        await interaction.editReply({
            content: `Leaderboard ${lb.lb_name} updated to be tracked with the role <@&${new_role.id}>. Run /update to reflect this change in role holders.`,
