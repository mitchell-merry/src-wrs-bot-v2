...
export const hasSubcommands = (
    cmd: Command | CommandWithSubcommands,
): cmd is CommandWithSubcommands => 'subcommands' in cmd;

export async function handleSlashCommand(
    interaction: ChatInputCommandInteraction,
) {
    const guildLog = (s: string) =>
        console.log(`[${interaction.guildId!}] ${s}`);
    guildLog(`Command receieved by ${interaction.user.tag}.`);

    const command = [...commands, AdminCommand].find(
    ...
    if (hasSubcommands(command)) {
        const sc = command.subcommands.find(
            sc => sc.data.name === interaction.options.getSubcommand(),
        );
        if (!sc)
            throw new Error(
    ...
    const userIsMod =
        userIsAdmin ||
        (await isUserMod(interaction.guildId, interaction.member));
    guildLog(
        `userIsSuperAdmin: ${userIsSuperAdmin}, userIsAdmin: ${userIsAdmin}, userIsMod: ${userIsMod}`,
    );

    // Check user has correct permission.
    if (perm === 'superadmin' && !userIsSuperAdmin)
        throw new UserError(
            `Only the best of the best (super-admins) can use this... for shame.`,
        );
    else if (perm === 'admin' && !userIsAdmin)
        throw new UserError(
            `Only admins are allowed to use this command! Loser. Scram!!`,
        );
    else if (perm === 'mods' && !userIsMod)
        throw new UserError(
            `Only mods and above are allowed to use this! Shame on you. Bad.`,
        );
    ...
    if (!guildEnt)
        throw new Error(
            'Guild not being tracked as an entity for some reason.',
        );
...
export async function handleAutocomplete(interaction: AutocompleteInteraction) {
    ...
    if (!acl)
        throw new Error(`Error getting autocomplete: Missing autocompleter`);
