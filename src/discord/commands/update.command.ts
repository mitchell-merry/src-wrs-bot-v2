...
const UpdateCommand: Command = {
    ...
    execute: async (interaction, guildEnt) => {
        ...
        const updateLog = (s: string) =>
            console.log(`[${interaction.guildId}] [/update] ${s}`);
        updateLog(
            `Update run by ${interaction.user.tag}, fetching all members and channels in guild...`,
        );
        ...
        if (
            ...
            throw new Error(
                "log_channel is either set to a channel which doesn't exist or a non-text channel. Run /set log_channel.",
            );
        ...
        guildEnt.trackedLeaderboards.forEach(tlb => {
            if (!roleLeaderboards[tlb.role_id])
                roleLeaderboards[tlb.role_id] = [];
        ...
        for (const [roleId, tlbs] of Object.entries(roleLeaderboards)) {
            ...
            if (!role) {
                ...
                let err =
                    `The role attached to the leaderboards ${lbs} no longer exists. ` +
            ...
            roleLog(
                `Member(s) with role: ${Array.from(role.members)
                    .map(([id, m]) => `${m.user.tag} (${id})`)
                    .join(', ')}`,
            );
            ...
            await Promise.all(
                tlbs.map(async tlb => {
                    const lblog = (s: string) =>
                        roleLog(`:: [${tlb.leaderboard.lb_name}] ${s}`);
                    ...
                    const partial: SRC.LeaderboardPartial = {
                        ...
                        variables: Object.fromEntries(
                            tlb.leaderboard.variables.map(variable => [
                    ...
                    const lb = await SRC.getLeaderboardFromPartial(
                        partial,
                        { top: 1 },
                        { cache: false },
                    ).catch(e => {
                        ...
                        if (e instanceof SRC.SRCError) {
                            err +=
                                `speedrun.com error: ${e.error.message} [${e.error.status}]\n` +
                    ...
                    // guests are ignored
                    const srcPlayerIds = lb.runs
                        .map(run =>
                            run.run.players
                                .filter(SRC.playerIsUser)
                                .map(p => p.id),
                        )
                        .flat();
                    lblog(
                        `Found WR holder(s) (sr.c): ${srcPlayerIds.join(', ')}`,
                    );

                    const discPlayerIDs = srcPlayerIds
                        .map(id =>
                            guildEnt.players.find(p => p.player_id === id),
                        )
                    ...
                    lblog(
                        `Found WR holder(s) (discord): ${discPlayerIDs.join(
                            ', ',
                        )}`,
                    );
                    ...
                    if (log_channel) {
                        const word =
                            discPlayerIDs.length === 1 ? 'has' : 'all have';
                        const embed = new EmbedBuilder()
                            .setColor(
                                role?.color ??
                                    guildEnt.role_default_colour ??
                                    '#FEE75C',
                            )
                            .setTitle(
                                `World Record in ${tlb.leaderboard.lb_name}`,
                            )
                            .setDescription(
                                `Now ${discPlayerIDs
                                    .map(id => `<@${discPlayerIDs}>`)
                                    .join(', ')} ${word} the world record!`,
                            );
            ...
            await Promise.all(
                role.members.map(async member => {
                    if (accounts.includes(member.id))
                        accounts = accounts.filter(a => a !== member.id);
                    else {
            ...
            await Promise.all(
                accounts.map(async a => {
                    ...
                    if (!user) {
                        roleLog(
                            `Account ${a} not in the server, removing association!`,
                        );
                        await DB.getRepository(PlayerEntity).delete({
                            discord_id: a,
