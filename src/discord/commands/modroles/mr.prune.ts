...
const ModrolesPruneCommand: Subcommand = {
    data: new SlashCommandSubcommandBuilder()
        .setName('prune')
        .setDescription(
            'Removes all deleted (stale) roles from the modrole list.',
        ),
    perm: 'admin',
    execute: async (interaction, guildEnt) => {
        ...
        const deletedRoles = guildEnt.moderatorRoles.filter(
        ...
        await interaction.editReply(
            `Pruned ${deletedRoles.length} stale roles. Thank you for shopping at Woolies.`,
        );
    },
};
