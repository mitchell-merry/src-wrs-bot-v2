...
const ModrolesAddCommand: Subcommand = {
    data: new SlashCommandSubcommandBuilder()
        ...
        .addRoleOption(o =>
            o.setName('role').setDescription('The role.').setRequired(true),
        ),
    perm: 'admin',
    execute: async (interaction, guildEnt) => {
        const roleOption = interaction.options.getRole('role', true);

        const has = guildEnt.moderatorRoles.find(
            mr => mr.role_id === roleOption.id,
        );
        if (!!has)
            throw new UserError(
                `That role is already set as a moderator in this guild.`,
            );

        const role = new ModeratorRoleEntity(
            interaction.guildId!,
            roleOption.id,
        );
