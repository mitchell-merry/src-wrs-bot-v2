...
export default class PaginatedList {
    ...
    constructor(
        items: string[],
        pageSize: number = 25,
        expireMessage = 'This list has expired.',
    ) {
        ...
        this.items.forEach(item => {
            ...
            const pageLength = lastPage.reduce(
                (prev, curr) => prev + curr.length,
                0,
            );

            if (
                lastPage.length >= this.pageSize ||
                pageLength + item.length > MESSAGE_LIMIT
            )
    ...
    public async spawnMenu(interaction: ChatInputCommandInteraction) {
        ...
        while (true) {
            ...
            await (r ? r.update(data) : interaction.editReply(data));
            ...
            r = await message
                ...
                .catch(_ => {
                    throw new UserError(this.expireMessage);
                });
    ...
    private getPageComponents(page: number) {
        ...
        return [
            new ActionRowBuilder<ButtonBuilder>().addComponents([
                new ButtonBuilder()
                    .setLabel('|<')
                    .setCustomId(`left_all`)
                    .setDisabled(isFirstPage)
                    .setStyle(ButtonStyle.Primary),
                new ButtonBuilder()
                    .setLabel('<')
                    .setCustomId(`left_once`)
                    .setDisabled(isFirstPage)
                    .setStyle(ButtonStyle.Primary),
                new ButtonBuilder()
                    .setLabel(`${page + 1} / ${this.pages.length}`)
                    .setCustomId('page')
                    .setDisabled(true)
                    .setStyle(ButtonStyle.Secondary),
                new ButtonBuilder()
                    .setLabel('>')
                    .setCustomId(`right_once`)
                    .setDisabled(isLastPage)
                    .setStyle(ButtonStyle.Primary),
                new ButtonBuilder()
                    .setLabel('>|')
                    .setCustomId(`right_all`)
                    .setDisabled(isLastPage)
                    .setStyle(ButtonStyle.Primary),
            ]),
        ];
