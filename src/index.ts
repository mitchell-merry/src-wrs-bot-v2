...
client.on('ready', async () => {
    ...
    console.log(
        `Logged in as ${client.user.tag} and in the following Guilds: [${client.guilds.cache.size}]`,
    );
    ...
    if (
        process.env.DiscordSyncCommands !== 'false' &&
...
for (const vari of requiredEnvs) {
    if (!process.env[vari])
        throw new Error(
            `Environment variable ${vari} is missing! Aborting startup.`,
        );
}
