...
export class LeaderboardEntity {
    ...
    @OneToMany(() => VariableEntity, variable => variable.leaderboard, {
        cascade: true,
    })
    ...
    static async exists(
        ...
        const possibles = await lRepo.find({
            where: { game_id, category_id, level_id },
            relations: { variables: true, trackedLeaderboards: true },
        });

        // check to see if any have all variables match
        return possibles.find(board =>
            board.variables.every(varEnt =>
                variables.find(
                    ([variable, value]) =>
                        varEnt.variable_id === variable.id &&
                        varEnt.value === value,
                ),
            ),
        );
